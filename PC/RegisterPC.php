            ...
            if(!empty($_POST['email1']) && !empty($_POST['email2']) || !empty($_POST['pass1']) && !empty($_POST['pass2'])){
              if ($_POST['email1']==$_POST['email2'] && !empty($_POST['email1']) && !empty($_POST['email2'])){
                if($_POST['pass1']==$_POST['pass2'] && !empty($_POST['pass1']) && !empty($_POST['pass2'])){
                    ...
                    $reponse = $bdd->query("SELECT (SELECT COUNT(*) FROM `USER` WHERE `Pseudo`='$pseudo') AS Pseudo,(SELECT COUNT(*) FROM USER WHERE `Email`='$email') AS mail");
                    ...
                    if($sameEmail == 0 && $samePseudo == 0) {
                      $link = mysqli_connect("127.0.0.1", "phpmyadmin", "crea-campus", "Medilik");
                      if($link === false){
                          die("ERROR: Could not connect to database");
                      }
                      $sql = "INSERT INTO USER (Pseudo, MDP, Email) VALUES ('$pseudo', '$MDP', '$email')";
                      if(mysqli_query($link, $sql)){
                        echo "Votre compte est créé !";
                      } else{
                        echo "ERROR: requete a la bdd ligne 88";
                      }
                      mysqli_close($link);
                    }
                    echo $erreurSame;
